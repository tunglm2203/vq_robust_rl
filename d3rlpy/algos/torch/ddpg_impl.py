...
class DDPGBaseImpl(ContinuousQFunctionMixin, TorchImplBase, metaclass=ABCMeta):
    ...
    @abstractmethod
    def _build_actor(self, **kwargs) -> None:
        pass
...
class DDPGImpl(DDPGBaseImpl):
    ...
    def _build_actor(self, use_vq_in: bool = False, number_embeddings: int = 128, embedding_dim: int = 1) -> None:
        self._policy = create_deterministic_policy(
            self._observation_shape,
            self._action_size,
            self._actor_encoder_factory,
            use_vq_in=use_vq_in, number_embeddings=number_embeddings, embedding_dim=embedding_dim
        )
