...
from ..adversarial_training.utility import get_stats_from_ckpt
...
def train_single_env(
    ...
    if finetune:
        ...
        algo._impl.load_model(checkpoint)
        LOG.debug("Load model successfully.")
        mean_ckpt, std_ckpt = get_stats_from_ckpt(checkpoint)
        if mean_ckpt is not None and std_ckpt is not None and algo._impl.scaler is not None:
            algo._impl.scaler._mean = mean_ckpt
            algo._impl.scaler._std = std_ckpt
            LOG.debug("Updated statistical from checkpoint.")

    # start training loop
    observation = env.reset()
    rollout_return = 0.0
    episode_length = 0
    if n_steps_collect_data == -1:
        n_steps_collect_data = n_steps
    for total_step in xrange(1, n_steps + 1):
        with logger.measure_time("step"):
            if total_step <= n_steps_collect_data:
            ...
            logger.add_metric("buffer_mean", algo._impl.scaler._mean.mean())
            logger.add_metric("buffer_std", algo._impl.scaler._std.mean())
