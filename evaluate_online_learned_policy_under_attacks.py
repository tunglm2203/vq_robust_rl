...
SUPPORTED_ATTACKS = ['none', 'random', 'minQ', 'minQ_rand', 'actor_mad', 'actor_mad_rand', 'sarsa']
...
def eval_func(algo, env, writer, attack_type, attack_epsilon, params):
    ...
    if multiprocessing:
        ...
        unorm_score_attack = eval_multiprocess_wrapper(algo, eval_env_under_attack_v1, env_list, _args)
        ...
        unorm_score_attack_list = eval_env_under_attack_v1(func_args)
        unorm_score_attack = unorm_score_attack_list["unorm_score"]
        print("diff output: %.4f" % (unorm_score_attack_list["diff_outs"]))
    ...
    if env.env.spec.id in d4rl.infos.DATASET_URLS.keys():
        ...
        norm_score_attack = unorm_score_attack
...
def main(args):
    assert len(args.attack_type_list) == 1
    args.eval_logdir = f"./evaluation/evaluation_online/adversarial/{args.attack_type_list[0]}/attack_eps{args.attack_epsilon}/{ENV_NAME_MAPPING[args.dataset]}"
    ...
    for n in range(N):
        ...
        score_dict.update({
            ...
            str(args.attack_type_list[n]) + '-' + str(args.attack_epsilon) + '_unorm': unorm_score_attack[n].tolist()
        })
